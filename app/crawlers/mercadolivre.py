        ...
        for component in components:
            ...
            match comp_type:
                ...
                case "price":
                    ...
                    price_discount = (
                        component.get("price", {}).get("current_price", {}).get("value")
                    )
                    discount = (
                        component.get("price", {})
                        .get("discount_label", {})
                    ...
                    for _value_installment in (
                        ...
                        match _value_installment.get("type"):
                            case "label":
                                value = _value_installment.get("label", {}).get(
                                    "text", ""
                                )
                            case "price":
                                value = to_brl(
                                    _value_installment.get("price", {}).get("value")
                                )
...
async def get_dict_response(page: Page) -> dict:
    ...
    json_match = re.search(
        r"(_n\.ctx\.r\s{0,}=\s{0,})(.*)(;_n\.ctx\.r\.assets\.manifest)", html
    ).group(2)
    return json.loads(json_match)


async def get_products(
    browser: Browser, departament_code: str, product_limit: int
) -> list[Product]:
    ...
    async def open_page(browser, departament_code: str, page_number: int):
        ...
        await page.goto(
            f"https://www.mercadolivre.com.br/ofertas?category={departament_code}&page={page_number}"
        )
        return page

    tasks = [open_page(browser, departament_code, i + 1) for i in range(max_pages)]
    ...
    products: list[Product] = await asyncio.gather(
        *[process_products(response, page) for response, page in responses]
    )
