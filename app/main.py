...
async def ingestion(group_name: str, stores: list[Base], sem: asyncio.Semaphore):
    async def process_store(store: Base):
        async with sem:
            products = await store.exec()
            for index, product in enumerate(products):
                key = f"{group_name.replace(' ', '_')}:{store.name()}:{store.departament_code}:{index}"
...
groups = [
    {
        ...
        "stores": [
            Amazon("Brinquedos e Jogos", 20, 15, "Brinquedos para Bebês e Crianças Pequenas"),
...
async def send_products_group(wpp: Whatsapp, group_name: str, name_id: str, sem: asyncio.Semaphore):
...
async def main():
    ...
    tasks = [ingestion(group["name"], group['stores'], ingestion_sem) for group in groups]
